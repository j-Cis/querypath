...
fn scenariusz_1_pojedyncza_lokalizacja() {
	...
	let entry =
		PathsEntry::build(input).expect("Scenariusz 1: Folder ./tests/_/abc/ istnieje i powinien zostać zmapowany.");
...
fn scenariusz_3_alternatywa_zwykly_i_ukryty() {
	...
	let entry = PathsEntry::build(input).expect("Scenariusz 3: Obie gałęzie (.gh oraz abc) istnieją na dysku.");

	assert_eq!(entry.targets.len(), 2, "Powinny zostać zmapowane dokładnie 2 lokalizacje.");

	let paths: Vec<&str> = entry.targets.iter().map(|t| t.relative_path.as_str()).collect();
...
fn scenariusz_4_alternatywa_rozne_glebokosci() {
	...
	let entry = PathsEntry::build(input).expect("Scenariusz 4: Obie gałęzie o różnych głębokościach istnieją.");
...
fn scenariusz_5_alternatywa_z_niewlasciwa_sciezka() {
	...
	let entry =
		PathsEntry::build(input).expect("Scenariusz 5: Powinien przetrwać istniejący cel, a błędny zostać odrzucony.");

	assert_eq!(entry.targets.len(), 1, "Nieistniejący podfolder dw/abc/ musi zostać cicho odrzucony.");
	assert!(entry.targets[0].relative_path.ends_with("tests/_/.gh/lib/dw/"));
}

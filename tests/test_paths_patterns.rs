use querypath::{FsWalk, PathsEntry, PathsPatterns, PattEnvIndex};
...
fn scenariusz_1_dopasowanie_globo_klamrowe() {
	...
	let matched: Vec<&str> = walk.files.iter().map(|f| f.str.as_str()).filter(|f| patterns.is_match(f, &env)).collect();
...
fn scenariusz_4_tylko_katalogi() {
	...
	let matched_dirs: Vec<&str> =
		walk.dirs.iter().map(|d| d.str.as_str()).filter(|d| patterns.is_match(d, &env)).collect();
	...
	let matched_files_count = walk.files.iter().filter(|f| patterns.is_match(&f.str, &env)).count();
...
fn scenariusz_5_ochrona_znaki_specialne_w_fizycznych_sciezkach() {
	...
	let matched_rt: Vec<&str> =
		walk.files.iter().map(|f| f.str.as_str()).filter(|f| patterns_rt.is_match(f, &env)).collect();
	...
	let matched_copy: Vec<&str> =
		walk.files.iter().map(|f| f.str.as_str()).filter(|f| patterns_copy.is_match(f, &env)).collect();

...
fn scenariusz_2_ignore_case_dopasowanie() {
	...
	let matched_strict: Vec<&str> =
		walk.files.iter().map(|f| f.str.as_str()).filter(|f| patterns_strict.is_match(f, &env)).collect();
	...
	let matched_ignore: Vec<&str> =
		walk.files.iter().map(|f| f.str.as_str()).filter(|f| patterns_ignore.is_match(f, &env)).collect();
...
fn scenariusz_3_keep_parent_katalogi_nadrzedne() {
	...
	let matched_files: Vec<&str> =
		walk.files.iter().map(|f| f.str.as_str()).filter(|f| patterns.is_match(f, &env)).collect();

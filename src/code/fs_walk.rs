...
impl FsWalk {
	...
	pub fn scan(entry: &PathsEntry) -> Result<Self> {
		...
		Ok(Self { files, dirs, stat: FsWalkStat { count_files, count_dirs, count_empty_dirs, count_empty_files } })
	}
		...
		for entry in WalkDir::new(root).into_iter().filter_map(|e| e.ok()) {
			...
			if entry.file_type().is_dir() {
				*count_dirs += 1;
				...
				let is_empty = entry.path().read_dir().map(|mut r| r.next().is_none()).unwrap_or(false);
